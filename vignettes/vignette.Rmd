...
* ```doNegativeControl```: (Optional) A __boolean__ specifying whether to do the negative control bootstrap resampling for each signature/dataset combination; note that if true, it will take significantly longer to run _sigQC_
* ```numResampling```: (Optional) A __numeric__ value specifying the number of bootstrapping runs to perform for the negative control
